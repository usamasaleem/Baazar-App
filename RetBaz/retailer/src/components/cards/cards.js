        ...
        return (
            <>
                ...
                <CardText className="CardText">1</CardText>
                    ...
                    <Edit>
                    <Popup   trigger={<DeleteButton><FontAwesomeIcon icon={ faEdit }/></DeleteButton> } modal>
                    {close => (
                      <Modal >
                        <Close onClick={close}>
                          &times;
                        </Close>
                        <Header > <Heading>{this.props.item.name}</Heading></Header>
                        <Content >
                        <Form>
                            <Photo>
                            <IconInput  src={`http://localhost:4000/uploads/${this.props.item.fileName}`}></IconInput>
                            </Photo>           
                            <Inputs>
                            <InputContainer>
                            
                            <Input type='text' placeholder="Cartons"  value={this.state.cartons} name="stock" onChange={e => this.setState({cartons:e.target.value})}></Input>
                            
                    </InputContainer>
                    <InputContainer>
                    <Input type='text' placeholder="Items/Cartons"  value={this.state.name} name="items" onChange={e => this.setState({items:e.target.value})}></Input>
                        
                            
                    </InputContainer>

                    <InputContainer style={{marginBottom:"5px"}}>
                    <Button onClick={() => this.editProduct(this.props.item._id)}>Update</Button>
                    ...
                    <DeleteButton onClick={() => this.editProduct(this.props.item._id)} ><FontAwesomeIcon  icon={ faTrashAlt }/></DeleteButton>

                </Cardcontainer>
               {this.state.isExpanded &&
                <ExpandedCard value={this.props.item} />    }
        </ >
